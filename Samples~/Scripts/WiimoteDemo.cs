...
namespace WiimoteDemo
{
    public class WiimoteDemo : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            GUI.Box(new Rect(0, 0, window_size + 20, Screen.height), "");

            GUILayout.BeginVertical(GUILayout.Width(window_size));
            ...
            for (int x = 0; x < 4; x++)
            {
                if (GUILayout.Button("" + x, GUILayout.Width(window_size / 4)))
                {
            ...
            GUILayout.BeginHorizontal();
            var size4 = GUILayout.Width(window_size / 4);
            if (GUILayout.Button("But/Acc", size4)) wiimote.SendDataReportMode(InputDataType.REPORT_BUTTONS_ACCEL);
            if (GUILayout.Button("But/Ext8", size4)) wiimote.SendDataReportMode(InputDataType.REPORT_BUTTONS_EXT8);
            if (GUILayout.Button("B/A/Ext16", size4)) wiimote.SendDataReportMode(InputDataType.REPORT_BUTTONS_ACCEL_EXT16);
            if (GUILayout.Button("Ext21", size4)) wiimote.SendDataReportMode(InputDataType.REPORT_EXT21);
            GUILayout.EndHorizontal();

            if (GUILayout.Button("Request Status Report"))
            {
                Debug.Log(wiimote.SendStatusInfoRequest());
            }

            GUILayout.Label("IR Setup Sequence:");
            GUILayout.BeginHorizontal();
            var size3 = GUILayout.Width(window_size / 3);
            if (GUILayout.Button("Basic", size3)) wiimote.SetupIRCamera(IRDataType.BASIC);
            if (GUILayout.Button("Extended", size3)) wiimote.SetupIRCamera(IRDataType.EXTENDED);
            if (GUILayout.Button("Full", size3)) wiimote.SetupIRCamera(IRDataType.FULL);
            GUILayout.EndHorizontal();
            ...
            for (int x = 0; x < 3; x++)
            {
                ...
                if (GUILayout.Button(step.ToString(), GUILayout.Width(window_size / 3)))
